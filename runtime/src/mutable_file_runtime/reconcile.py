...
def _apply_resolution_run(run: DocumentRun, repo: GitStateRepo) -> str:
    ...
    if resolve_text is None or local_text is None:
        raise RuntimeError("resolve result does not match current tasks; update tasks or abort the session")
    ...
    if resolve_value != run.desired_managed:
        ...
        raise RuntimeError(f"resolve result does not match current tasks; update tasks or abort the session: {details}")
    if run.current_local_view != local_view:
        raise RuntimeError("local file changed since the resolve session was created; abort and rerun")
    ...
    if verified_view != resolve_value:
        raise RuntimeError("resolved render did not match the resolve branch semantic output")
    return rendered
...
def reconcile_documents(documents):
    ...
    if len(state_dirs) != 1:
        raise ValueError("reconcile_documents expects documents from exactly one state_dir")

    repo = GitStateRepo(documents[0].state_dir)
    repo.ensure_initialized()

    if repo.worktree_merge_in_progress():
        raise RuntimeError("resolve worktree is still in a merge state; finish the merge or abort it")
    ...
    if first_conflict is not None:
        ...
        raise RuntimeError("conflict detected; review and resolve it in the resolve worktree")
